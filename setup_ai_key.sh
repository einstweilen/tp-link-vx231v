...
echo -e "\n${BOLD}==== KI-Analyse Einrichtung ====${NC}"

EXISTING_KEY=""
if [[ -f "config.ini" ]]; then
    EXISTING_KEY=$(grep "^ai_api_key" config.ini | cut -d '=' -f2- | tr -d ' ' | tr -d '\r\n')
...
while true; do
    ...
    if [[ -n "$EXISTING_KEY" ]]; then
        ...
        info "Es ist bereits ein Gemini API Key hinterlegt (\"AIza…$LAST_FOUR\")"
    ...
    if [[ -z "$API_KEY" ]]; then
        ...
        if [[ "$AI_CHOICE" != "G" && "$AI_CHOICE" != "V" ]]; then
            ...
            if [[ -f "config.ini" ]] && grep -q "^\[AI\]" config.ini; then
                if [[ "$(uname)" == "Darwin" ]]; then
                    sed -i '' "s/^\[AI\]/\[noAI\]/g" config.ini
                else
                    sed -i "s/^\[AI\]/\[noAI\]/g" config.ini
                fi
...
EOF
    ...
    if python3 .test_api.py "$API_KEY"; then
...
info "Speichere API-Key in config.ini..."
if [ ! -f "config.ini" ]; then
    touch config.ini
fi

# Reaktivierung, falls [noAI] existiert
if grep -q "^\[noAI\]" config.ini; then
    if [[ "$(uname)" == "Darwin" ]]; then
        sed -i '' "s/^\[noAI\]/\[AI\]/g" config.ini
    else
        sed -i "s/^\[noAI\]/\[AI\]/g" config.ini
    fi
fi

if ! grep -q "^\[AI\]" config.ini; then
    echo -e "\n[AI] # AI = Modul aktiviert noAI = Modul deaktiviert\nai_provider = $PROVIDER\nai_api_key = $API_KEY" >> config.ini
else
    if grep -q "^ai_provider" config.ini; then
        if [[ "$(uname)" == "Darwin" ]]; then
            sed -i '' "s/^ai_provider.*/ai_provider = $PROVIDER/g" config.ini
            sed -i '' "s/^ai_api_key.*/ai_api_key = $API_KEY/g" config.ini
        else
            sed -i "s/^ai_provider.*/ai_provider = $PROVIDER/g" config.ini
            sed -i "s/^ai_api_key.*/ai_api_key = $API_KEY/g" config.ini
        fi
